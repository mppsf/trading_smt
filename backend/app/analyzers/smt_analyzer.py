...
class SMTAnalyzer(BaseAnalyzer):
    def analyze(self, es_data: List[OHLCV], nq_data: List[OHLCV]) -> List[Signal]:
        es_highs, es_lows = self._get_fractals(es_data)
        nq_highs, nq_lows = self._get_fractals(nq_data)
        
        if len(es_lows) < 2 or len(nq_lows) < 2 or len(es_highs) < 2 or len(nq_highs) < 2:
            return []
        
        signals = []
        threshold = self.settings.get('divergence_threshold', 0.5)
        ...
        if es_lows[-2][1] < es_lows[-1][1] and nq_lows[-2][1] > nq_lows[-1][1]:
            ...
            if div_pct >= threshold:
                signals.append(Signal(
                    ...
                    confirmed=self._check_confirmation(es_data, nq_data, 'bullish'),
        ...
        if es_highs[-2][1] > es_highs[-1][1] and nq_highs[-2][1] < nq_highs[-1][1]:
            ...
            if div_pct >= threshold:
                signals.append(Signal(
                    ...
                    nq_price=nq_data[-1].close,
                    divergence_pct=div_pct,
                    confirmed=self._check_confirmation(es_data, nq_data, 'bearish'),
                    details={'threshold': threshold}
                ))
        
        return signals
    
    def _check_confirmation(self, es_data: List[OHLCV], nq_data: List[OHLCV], direction: str) -> bool:
        conf_candles = self.settings.get('confirmation_candles', 3)

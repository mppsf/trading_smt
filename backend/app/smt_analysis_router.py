...
async def get_smt_signals(
    ...
    try:
        ...
        if smt_strength_threshold is not None:
            custom_params['smt_strength_threshold'] = smt_strength_threshold
        if divergence_threshold is not None:
            custom_params['divergence_threshold'] = divergence_threshold
        if confirmation_candles is not None:
            custom_params['confirmation_candles'] = confirmation_candles
        if volume_multiplier is not None:
            custom_params['volume_multiplier'] = volume_multiplier
        if max_signals_display is not None:
            custom_params['max_signals_display'] = max_signals_display
        if refresh_interval is not None:
            custom_params['refresh_interval'] = refresh_interval
        if london_open is not None:
            custom_params['london_open'] = london_open
        if ny_open is not None:
            custom_params['ny_open'] = ny_open
        if asia_open is not None:
            custom_params['asia_open'] = asia_open
        if killzone_priorities is not None:
            try:
                priorities = [int(p.strip()) for p in killzone_priorities.split(',')]
                custom_params['killzone_priorities'] = priorities
            except ValueError:
        ...
        # Создаем сервис и выполняем анализ с кастомными параметрами
        smt_service = SmartMoneyService()
        
        if custom_params:
            # Выполняем свежий анализ с кастомными параметрами
            market_data = {'ES=F': es_data, 'NQ=F': nq_data}
            signals = await smt_service.analyze(market_data, custom_params)
        else:
        ...
        final_limit = min(limit, custom_params.get('max_signals_display', limit))
